...
| BL-009 | Auto-Approve for Workspace Files | Medium | Add a configurable flag (e.g., `AUTO_APPROVE_WORKSPACE=true`) that skips human approval for file read/write operations within the workspace. When enabled, the agent can read and write files freely without prompting, relying on git branch isolation (BL-008) as the safety net. Shell commands and delete operations should still require approval. Flag should be configurable via `.env` and `/config` CLI command at runtime. Disabled by default for zero-tolerance security posture. |
